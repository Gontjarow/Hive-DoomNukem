...
/*   Updated: 2021/08/18 17:49:14 by msuarez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	bullet_moving_validation(int last_id, t_enemy *enemy, t_doom *doom)
{
	...
	current_room_id = room_id_from_polymap(doom->mdl->poly_map,
			enemy->bullet_pos.x, enemy->bullet_pos.y);
	if (current_room_id != last_id)
	{
		if (check_bullet_hitting_door(doom, cached_bullet_loc
				, (t_xy){enemy->bullet_pos.x, enemy->bullet_pos.y}))
			return (-1);
	}
	return (0);
}
...
void	enemy_shoot_the_player(t_doom *doom, t_enemy *enemy)
{
	...
	last_room_id = room_id_from_polymap(doom->mdl->poly_map,
			enemy->x, enemy->y);
	while (check_location(doom, enemy->bullet_pos.x,
			enemy->bullet_pos.y) != -1 && did_hit == 0)
	{
		if (bullet_moving_validation(last_room_id, enemy, doom) == -1)
			break ;
