...
void	add_enemy_to_string(t_enemy *enemy, t_mapfile *map)
{
	...
	sprintf(map->join_string, "[Enemy] id = %d | x = %d | y = %d | rot = %d | hp = %d | wep.type_id = %d | ai.type_id = %d\n",
			enemy->id, enemy->x, enemy->y, enemy->rot, enemy->hp.max, enemy->wep.type_id, enemy->ai.type_id);
	if (!map->enemy_string)
