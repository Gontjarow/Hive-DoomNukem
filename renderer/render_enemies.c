...
void			render_enemies(t_doom *doom)
{
	...
	while (~--enemy_count)
	{
		...
		if (eline.start.y <= NEAR_PLANE)
		{
			...
			double ceil = world->sectors[enemy_id].floor / WORLD_SCALE + EYE_HEIGHT - world->player.position.z;
			double floor = world->sectors[enemy_id].floor / WORLD_SCALE - 0.5 - world->player.position.z;
