...
// mockup elevator effect type
// [effect] id = 1 | type_id = 1 (EFFECT_ELEVATOR) | lox.x = button.x | loc.y = button.y
// | target.x = min_height | target.y = max_height | target_id = 0 (nothing) target_id = 1 (going up)

//	RECENTLY COMPLETED
//		CHECK THAT CHAINING MAPS WORKS
//		CHECK THAT USING LEVEL EXITS WORKS
//		LOOK IF POSSIBLE TO FIX APPLY_GRAVITY
//		CHECK THAT CHAINED PROPAGATES TO MODEL INGAME FOR MAPS THAT HAVE IT
//		USE CHAINED VALUE TO OVERWRITE ADDED_ARG
//		CHECK THAT ADDED_ARG METHOD WORKS FOR CHAINS
//		LOADING SCREEN FOR STORY

// TODO FIXES
//		MODIFY THE CODE SO THAT LEVEL EXIT CHAINING DOES THE DELOAD/RELOAD CYCLE AUTOMATICALLY IN SEQUENCE
//		DISPLAY SPRITES FOR PICKUPS AND LEVEL EXIT
//		PLAY SOUNDS AT TYPEWRITER
//		PLAY MUSIC AT LOADING SCREENS, GAME
//		FIX THE MOST COMMON EDITOR BUGS

// TODO MANDATORIES
//		DIFFERENT CROSSHAIRS FOR WEAPONS
//		INCLINED FLOOR AND ROOF AKA SLOPING ROOMS (HINGE OFF A SINGLE WALL OF A ROOM)
//		SKYBOX
//		ARGUE THAT PORTALS == WINDOWS ?
//		BIND UP AND DOWN TO FORWARD AND BACKWARD PER THE MANDATORY SPEC

//	TODO
//		ADD KNEE_HEIGHT MANAGEMENT FOR ENTERING ROOMS THAT ARE KNEEHEIGHT RANGE HIGHER THAN FLOOR HEIGHT
//		ADD JUMP ENTERING ROOMS WHERE POSSIBLE
//		ADD BLOCKING OF ENTERING ROOMS WHEN NOT ENOUGH ROOM TO ENTER
//      DEFERRED ENABLE TEXTURING ROOMS IN EDITOR
//		DEFERRED ENABLE JOINED WALL PORTAL CREATION
//		DEFERRED CREATE A DIGITAL CLOCK STYLE FONT WITH THE DIGITAL CLOCK PROCEDURAL GENERATION IDEA
...
//			FIRST EFFECTOR IS LEVEL EXIT THAT EXITS LEVEL AND STARTS LOADING THE NEXT CHAINED LEVEL - 							LEVEL EXIT
//			SECOND EFFECTOR WILL BE AN ELEVATOR EFFECTOR THAT RAISES THE ROOM FLOOR AND CEILING THAT IT EXISTS IN - 			ELEVATOR
//			THIRD EFFECTOR WILL BE A KEY THAT CAN BE PICKED UP TO ENABLE OPENING OF DOORS - 									DOOR KEY
//			FOUR EFFECTOR WILL BE LIGHT SOURCES WITH LIGHT INTENSITY AND RADIUS VALUES - 										LIGHT SWITCH
//			FIFTH EFFECTOR WILL BE A SWITCH, WITH LOCATION AND TARGET TYPE AND ID (TOGGLING LIGHT OR OPENING BLOCKING DOOR) - 	OPENER SWITCH
//				NOTE: FOR A BLANK CHAIN_TO_MAPFILE VALUE THE MAP CAMPAIGN TRIGGERS CAMPAIGN VICTORY FROM THE LEVEL EXIT
